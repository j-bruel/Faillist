...
void    write_report(int fd, struct report *report)
{
    ...
    write(1, "Faillist: Updated output file.\n", strlen("Faillist: Updated output file.\n"));
    fstat(fd, &st);
    if (report == NULL ||
        fd < 1)
        return ;
    ...
    for (size_t i = 0; i < report->nb_failures; ++i)
        if (!write_failure(fd, i, &report->faillures[i]))
            write(1, "Faillist: Warning, failure has been impossible to write on the output file.\n", strlen("Faillist: Warning, failure has been impossible to write on the output file.\n"));
}